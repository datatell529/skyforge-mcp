...
try:
    ...
    logger.error("Check connection settings (host, port, credentials) and SkySpark server availability")
    skyspark = None
...
async def _handle_axon_tool_call(axon_tool: types.Tool, req: types.CallToolRequest) -> types.ServerResult:
    arguments = req.params.arguments or {}
        ...
        return types.ServerResult(
            types.CallToolResult(
                content=[
                    types.TextContent(
                        type="text",
                        text=f"Tool {axon_tool.name} is not a valid axon tool",
                    )
                ],
                isError=True,
            )
    ...
    try:
        hgrid_result = skyspark.handleToolCall(axon_tool.name, arguments, params_kind, params_order)
        ...
        return types.ServerResult(
            types.CallToolResult(
                content=[
                    types.TextContent(
                        type="text",
                        text=f"Axon tool execution failed: {str(e)}",
                    )
                ],
                isError=True,
            )
